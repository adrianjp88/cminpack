...
/* L50: */
    ...
    for (i__ = 1; i__ <= i__1; ++i__) {
	temp = 1.f;
	if (fvec[i__] != 0.f && fvecp[i__] != 0.f && fabsf(fvecp[i__] - 
		fvec[i__]) >= epsf * fabsf(fvec[i__]))
		 {
	    temp = eps * fabsf((fvecp[i__] - fvec[i__]) / eps - err[i__]) 
		    / (fabsf(fvec[i__]) +
                       fabsf(fvecp[i__]));
	}
	err[i__] = 1.f;
	if (temp > epsmch && temp < eps) {
	    err[i__] = (log10e * logf(temp) - epslog) / epslog;
	}
	if (temp >= eps) {
	    err[i__] = 0.f;
	}
